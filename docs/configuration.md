...
| `POST /push/dispatch` | `POST /api/push/dispatch` | `X-Dispatch-Key` equal to `DISPATCH_KEY` | `DISPATCH_KEY`, `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT`, `SUBS` | `200 {"delivered"}`; `401` `authentication` problem on a bad key; `503` when either VAPID key is unset; `400` `bad-request` problem when the body is not JSON. Subscriptions answering `403`, `404` or `410` are deleted |
...
| `GET /watches/watchers?item=` | `GET /api/watches/watchers?item=` | `X-Dispatch-Key` equal to `DISPATCH_KEY` | `DISPATCH_KEY`, `SUBS` | `200 {"uuids"}`; `401` `authentication` problem on a bad key; `400` `bad-request` problem on a malformed item |
