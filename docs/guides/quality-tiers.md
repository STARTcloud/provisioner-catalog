...
| `silver.lint_ci`                   | The string `ansible-lint` appears in the concatenated text of the repository's `.github/workflows/*.yml` and `*.yaml` files (first 20 files, first 256 KiB of each). An unreadable or missing workflows directory fails this rule |
...
| Rule                        | Measured condition                                                                                                                                                                                                                                                                                                                                                                       |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `platinum.automated_tests`  | The manifest declares roles, at least `max(1, ceil(roles × 0.5))` of those declared roles carry a substantive molecule scenario in the latest archive, and the string `molecule` appears in the repository's workflow text. A declared role counts as tested only when some archive member path contains `/roles/<name>/molecule/` and that scenario holds a `molecule.yml` **and** a `converge.yml` or `verify.yml`. Test trees of vendored collections whose roles the manifest does not declare count for nothing; an empty `tests/.keep` scores zero |
...
| `silver.lint_ci`                 | Run `ansible-lint` in a workflow under `.github/workflows/` (the reference repository's `ci.yml` does)                                                                                                          |
...
| `platinum.automated_tests`       | For at least half of the roles the manifest declares (minimum one), add a `molecule.yml` plus a `converge.yml` or `verify.yml` under that role's `molecule/` directory inside the shipped archive, and run `molecule` in a workflow under `.github/workflows/` |
